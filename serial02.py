...
myComport = None
myComportname = None
isCommandResponse = False  # isCommandResponceをisCommandResponseに修正
...
ser = serial.Serial('COM3', 9600, parity='N',timeout=2)
...
# GPSデータの初期化処理
def getGPSinitdata():
    global isCommandResponse, status_gps
...
def getgpsdata():
    ...
    msec = 1.0  # 間隔の設定
    gpstimer = threading.Timer(msec, sendGPScommand, args=(gps_command_senddata,))  # 引数を追加
    gpstimer.start()  # タイマーの開始

# コマンド送信処理
def sendGPScommand(command):
    global receivebuf, isCommandResponse

    if isCommandResponse:  # iscommandresponceをisCommandResponseに修正
        return

    receivebuf = []
    isCommandResponse = True  # iscommandresponceをisCommandResponseに修正
    try:
...
while True:
    ...
    ports = getserialportlist()  # asyncioを使用せずに実行
    print(ports)